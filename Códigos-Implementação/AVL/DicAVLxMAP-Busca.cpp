...
//A consulta é idêntica a uma Árvore Binária
...
int main(){
    Timer tmr;
    int encontrouMapa=0, encontrouAVL=0; //Variáveis de controle
    double t = tmr.elapsed(); //Tempo de execução do trecho de código em segundos (s)
    int N = 15000000; //Número de nós da árvore/Mapa. Range 500.000-15.000.000
    int num_rodadas = 500000; //Número de rodadas para pegar a média de tempos e evitar distorções
    ...
    std::cout<<std::endl;
    //GeraAleatorios(numeros,num_rodadas,N);// Vetor com valores aleatórios para buscar

    //=============================Mapa============================================
    std::map<int, int> Mapa; //Para comparar com a implementação de árvore AVL
    ...
    for(int j=0;j<num_rodadas;j++)
    {
        tmr.reset();
        //======================Zera o contador. Código a ser medido deve estar depois dessa linha
        if(consulta_ArvAVL(avl,j)== 1) //Zero não encontrou, 1 encontrou
        {
        //======================Para o contador. Código a ser medido deve estar antes dessa linha
            t = tmr.elapsed();
            encontrouAVL=1; //Seta pra 1 pra verificar no final se encontrou o elemento sem problemas
        }
        tempo_AVL[j] = t;
    }
    ...
    std::cout << "Tempo medio para encontrar elemento na AVL com " << N << " nos = " << somaAVL/num_rodadas << " segundos" << std::endl; //Exibe na tela
    std::cout<<std::endl;
    free(tempo_AVL); //Libera o vetor

    if((encontrouAVL==0)||(encontrouMapa==0))
