...
int main(){
    ...
    int N = 2500000; //Número de nós da árvore/Mapa. Range 1.000-20.000.000
    ...
    //=============================Mapa============================================
    //Cria um Mapa adicionando os elementos na seguência: 0, 2, 3, 4...N-1
    std::cout<<"Inserindo elementos no Mapa e calculando o tempo..."<<std::endl;
    std::cout<<std::endl;
    for(int j=0;j<num_rodadas;j++)
    {
        std::map<int, int> Mapa; //Para comparar com a implementação de árvore LLRB. Redefino a cada rodada
        for(int i=0;i<N;i++){
            tmr.reset();
    ...
    free(tempo_MAPA);
    //=============================LLRB============================================
    std::cout<<"Inserindo elementos na LLRB e calculando o tempo..."<<std::endl;
    std::cout<<std::endl;
    for(int j=0;j<num_rodadas;j++)
    {
        ArvLLRB *raiz = cria_ArvLLRB();//Cria árvore LLRB
        for(int i=0;i<N;i++)
        {
            tmr.reset();
            //======================Zera o contador. Código a ser medido deve estar depois dessa linha
            res = insere_ArvLLRB(raiz,i,i);
            //======================Para o contador. Código a ser medido deve estar antes dessa linha
            t = tmr.elapsed();
            tempo_LLRB[i] = t;
        }
    ...
    std::cout<<"Calculando o tempo medio de insercao..."<<std::endl;
    std::cout<<std::endl;
    for(int j=0;j<N;j++) //Calcula o tempo total para inserir N valores na LLRB
    {
        somaLLRB = somaLLRB + tempo_LLRB[j];
    }
    std::cout << "Tempo medio para preencher LLRB com " << N << " nos = " << somaLLRB << " segundos" << std::endl; //Exibe na tela
    std::cout<<std::endl;
